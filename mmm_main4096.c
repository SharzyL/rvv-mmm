...
const int way = vl / word_bits / 2; // 64
...
# ifdef SCRATCHPAD
void mmm(uint32_t* r, const uint32_t* a, const uint32_t* b, const uint32_t* p, const uint32_t mu, uint32_t *base);
# else
...
int main() {
# ifdef SCRATCHPAD
  uint32_t *scratchpad = (uint32_t *) malloc((256 + 4) * sizeof(uint32_t));
  mmm(abr1, a, b, p, mu, scratchpad);
# else
  mmm(abr1, a, b, p, mu);
# endif
